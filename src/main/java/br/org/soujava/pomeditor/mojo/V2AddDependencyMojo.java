 ...
 */
@Mojo(name = "add-dep2")
public class V2AddDependencyMojo extends AbstractMojo {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        try {
            getLog().info(String.format("trying to add the dependency: %s to the \"%s\" file...", toString(), pomFile));
            Pom.from(pomFile)
            ...
            getLog().info(String.format("added the dependency: %s to the \"%s\" file.", toString(), pomFile));
        } catch (Throwable ex) {
